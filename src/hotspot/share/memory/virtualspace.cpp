...
bool VirtualSpace::initialize(ReservedSpace rs, size_t committed_size, bool coroutine_stack) {
  const size_t max_commit_granularity = os::page_size_for_region_unaligned(rs.size(), 1);
  return initialize_with_granularity(rs, committed_size, max_commit_granularity, coroutine_stack);
}

bool VirtualSpace::initialize_with_granularity(ReservedSpace rs, size_t committed_size, size_t max_commit_granularity, bool coroutine_stack) {
  ...
  if (committed_size > 0) {
    if (!expand_by(committed_size, false, coroutine_stack)) {
      return false;
    }
  }
  return true;
...
static bool commit_expanded(char* start, size_t size, size_t alignment, bool pre_touch, bool executable, bool coroutine_stack) {
  if (os::commit_memory(start, size, alignment, executable)) {
    if ((pre_touch || AlwaysPreTouch) && !coroutine_stack) {
...
*/
bool VirtualSpace::expand_by(size_t bytes, bool pre_touch, bool coroutine_stack) {
  ...
  if (lower_needs > 0) {
    ...
    if (!commit_expanded(lower_high(), lower_needs, _lower_alignment, pre_touch, _executable, coroutine_stack)) {
      return false;
    }
  ...
  if (middle_needs > 0) {
    ...
    if (!commit_expanded(middle_high(), middle_needs, _middle_alignment, pre_touch, _executable, coroutine_stack)) {
      return false;
    }
  ...
  if (upper_needs > 0) {
    ...
    if (!commit_expanded(upper_high(), upper_needs, _upper_alignment, pre_touch, _executable, coroutine_stack)) {
      return false;
    }
